...
inputs:
  ...
  feature_bc_matrices_folder:
    type: File
    doc: |
      Path to the compressed folder with feature-barcode matrix from Cell Ranger
      Count/Aggregate experiment in MEX format.
  ...
  dimensions:
    ...
    doc: |
      ...
      graph before clustering (from 1 to 50). If single value N is provided, use from 1
      to N dimensions. If multiple values are provided, subset to only selected dimensions.
...
outputs:
  ...
  ucsc_cb_html_data:
    type: File
    outputSource: compress_ucsc_cb_html_data/compressed_folder
    doc: |
      Compressed directory with UCSC Cellbrowser html data.
...
steps:
  ...
  compress_ucsc_cb_html_data:
    run: ../tools/tar-compress.cwl
    in:
      folder_to_compress: sc_rna_cluster/ucsc_cb_html_data
    out:
    - compressed_folder
